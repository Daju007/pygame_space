...
class entity:
    ...
    def generate_projectile(self, damage):
        ...
        if self.projectile_cooldown_count ==0:  
            ...
            if self.facing ==1:
                self.centre = [self.rect.x+66, self.rect.y+33]
            if self.facing ==-1:
...
for i in range(4):
    enemy_iteration+=1
    enemies.append(entity([random.randint(scroll[0], scroll[0] +1600),scroll[1]],2, 0, 0.5, 10, 13, 7, 5, 66, 66, 1, "idle", 0, [0,0], 5, False, False, enemy_iteration, 11,100,30, (255,0,0)))
...
for enemy in enemies:
    enemy_animation_database["run_left"] = enemy.load_animation("EL", [enemy.running_animation_speed,enemy.running_animation_speed,enemy.running_animation_speed,enemy.running_animation_speed,enemy.running_animation_speed,enemy.running_animation_speed])
        ...
        for enemy in enemies:
            if enemy.rect.colliderect(screen_rect):
                ...
                if enemy.moving_right == True:
                    if ((player.rect.x-66)-enemy.rect.x)<300:
                        enemy.fire=True
                elif enemy.moving_left==True:
                    if (enemy.rect.x-(player.rect.x+66))<300:
                        enemy.fire=True
                else: 
                    enemy.fire= False
                ...
                enemy.handle_projectiles(screen, scroll, player.projectile_image, player.projectile_image_flipped)
                ...
                enemy.frame += 1
                if enemy.frame >= len(enemy_animation_database[enemy.action]):
                    enemy.frame = 0
                enemy.image_id = enemy_animation_database[enemy.action][enemy.frame]
